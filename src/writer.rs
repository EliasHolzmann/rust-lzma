...
use error::LzmaError;
use lzma_stream_wrapper::{LzmaCodeResult, LzmaStreamWrapper};
use lzma_sys::*;
use std;
...
impl<T: Write> LzmaWriter<T> {
    ...
    pub fn with_capacity(
        ...
        match writer.direction {
            Direction::Compress => writer
                .stream
                .easy_encoder(preset, lzma_check::LzmaCheckCrc64)?,
            Direction::Decompress => writer.stream.stream_decoder(std::u64::MAX, 0)?,
        }
...
impl<W: Write> LzmaWriter<W> {
    ...
    fn lzma_code_and_write(
        &mut self,
        input: &[u8],
        action: lzma_action,
    ) -> Result<LzmaCodeResult, LzmaError> {
